...
2. 如何理解缓存穿透、缓存击穿、缓存雪崩、缓存预热，如何解决
...
8. Redis和数据库如何实现双写一致性

    {{< spoiler >}} 
    1）Cache Aside Pattern ：-读的时候，先读缓存，缓存没有的话，就读数据库，然后取出数据后放入缓存，同时返回响应。 - 更新的时候，先更新数据库，然后再删除缓存，这种方案实际上在高并发的时候可以继续进行优化
    {{< / spoiler >}}
