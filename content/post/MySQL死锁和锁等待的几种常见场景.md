...
- 多个事务，同时锁定同一个资源时，也会产生死锁
  ...
  由于死锁日志中，仅仅是部分信息，仅有冲突时候的SQL和部分锁的情况，因此还需要本地复现来详细分析死锁原因。
  本地可以通过IDEA中的DataBase插件来复现，连接数据库，新建几个Console，手动调整隔离级别与事务提交方式
  [图片]
  [图片]
...
看似不会这么写代码，针对同一张表先a后b与先b后a，但仍然有该情况的变种
- 两处代码，一处是先表A再表B，另外一处入口是先表B再表A
...
## 数据重叠 - 有辅助索引但没使用到
  上一个例子里，如果对sql1中的is_delete加一下索引能否解决问题呢？
  这里特意把隔离级别调成RR
...
如果多个事务中，当前读的sql锁定的数据中有重叠，也会出现锁等待。
...
继续讨论这个例子，如果把唯一索引从数字换成字符串，需要考虑字符串的大小比较规则了,如下图。
但是考虑到我司实践的情况,如果是单据号一类的字符串唯一索引，一般采用发号器，发号器产生的数据长度固定，且递增增长，因此单据号一类的insert相对不容易出问题。
[图片]
### 变种
